...
- **v0.5.48** — `sdiv` for `(int / const) | 0` + `@llvm.assume` bounds in Uint8ArrayGet. `BitOr(Div(a, b), Integer(0))` now emits `sdiv i32` directly when both operands are int-lowerable (LLVM converts to `smulh + asr`, ~2 cycles vs ~10 for fdiv). `Uint8ArrayGet` bounds check replaced with `call void @llvm.assume(i1 in_bounds)` — eliminates the branch+phi diamond, making the inner loop single-BB for the vectorizer. image_conv: 0.69s → 0.61s. Gap tests: 15 PASS.
